...
const initiateKhaltiPayment = (post, userId, orderId) => {
    const config = {
        publicKey: "test_public_key_dc74e9cd831546c7b074ccf76eb00e3a",  // Replace this with your Khalti test public key
        productIdentity: post._id,
        productName: post.postType,
        productUrl: `https://yourwebsite.com/rental/${post._id}`,  // Replace with the actual URL of the product
        eventHandler: {
            onSuccess(payload) {
                // On success, call the backend to verify the payment
                verifyKhaltiPayment(payload, post._id, userId, orderId);
            },
            onError(error) {
        ...
        paymentPreference: ["KHALTI"],  // Other options include "EBANKING", "MOBILE_BANKING", etc.
    };

    const checkout = new KhaltiCheckout(config);
    checkout.show({ amount: post.price * 100 });  // Khalti expects amount in paisa
};

// Helper function to call backend API for payment verification
const verifyKhaltiPayment = async (payload, postId, userId, orderId) => {
    try {
