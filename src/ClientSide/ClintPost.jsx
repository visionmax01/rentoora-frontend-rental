...
function ClientPost() {
  ...
  const getDistricts = () => {
    const province = locationData.find((prov) => prov.name === formData.province);
  ...
  const getMunicipalities = () => {
    ...
    return district ? [...district.municipalities, ...district.ruralMunicipalities] : [];
  };

  return (
    <div className="w-full bg-brand-bgColor h-auto pb-8 overflow-y-auto lg:px-4 pt-4">
      <div className="lg:w-[85%] ml-1 w-full mx-auto">
        <h1 className="lg:text-2xl text-md font-extrabold text-center mb-6">Post Your Room, Apartment, or House For Rent</h1>
        <div className="lg:flex w-full lg:gap-6 pr-2">
          ...
          <div className="bg-gray-700 p-6 rounded-lg mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <select
                  ...
                  className="mt-1 block w-full bg-transparent focus:border-red-600 border outline-none border-gray-300 text-white rounded-sm p-2"
                >
                  <option className="text-gray-300" value="">Select Province</option>
                  {locationData.map((prov) => (
                    <option className="text-black" key={prov.name} value={prov.name}>
                      {prov.name}
                    </option>
                  ))}
                </select>
              ...
              {formData.province && (
                <div>
                  ...
                  <select
                    ...
                    className="mt-1 block w-full bg-transparent focus:border-red-600 border outline-none border-gray-300 text-white rounded-sm p-2"
                  >
                    <option className="text-gray-300" value="">Select District</option>
                    {getDistricts().map((dist) => (
                      <option className="text-black" key={dist.name} value={dist.name}>
                        {dist.name}
                      </option>
                    ))}
                  </select>
              ...
              {formData.district && (
                <div>
                  ...
                  >
                    <option className="text-gray-300" value="">Select Municipality</option>
            ...
            {/* Landmark Field */}
            <div className="mt-4">
              <label htmlFor="landmark" className="block text-sm font-medium">Location Landmark</label>
              <input
                type="text"
                name="landmark"
            ...
            <div className="mt-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {[1, 2, 3].map((index) => (
                  <div key={index}>
                    <label htmlFor={`image${index}`} className="block text-sm font-medium">Image {index}</label>
                    <input
                      type="file"
                      name={`image${index}`}
                      accept="image/*"
                      onChange={handleFileChange}
                      className="block w-full bg-transparent focus:border-red-600 border outline-none text-white rounded-sm p-2"
                    />
